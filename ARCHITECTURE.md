...
- **區間比較**：兩個日期選擇器（起始／結束，內容來自 `manifest.dates`，預設抓最近約 20 個交易日），上市／上櫃各自顯示：加權（櫃買）指數漲跌點數／%、融資餘額變化金額／%、三大法人買賣超區間累計（把區間內每天的 `net` 加總），再加一張「相對起始日變化 %」的走勢圖（指數與融資餘額都換算成 % 疊在同一張圖，方便看兩者是不是同向）。全部在前端算，資料來源是已經載入的 `data/series/{index,margin,institutional}.json`，換區間不用重新打 API。
